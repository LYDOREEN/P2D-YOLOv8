...
from ultralytics.nn.modules import (
    ...
    CSPStage,
    LSKA, C2f_SCConv, C2f_Faster_EMA, C2f_EMA1, C2f_EMA2, C2f_ODConv, CoordAtt, GAM, SPPF_LSKA,SCAM,
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    base_modules = frozenset(
        {
            ...
            CSPStage,
            C2f_SCConv,
            C2f_Faster_EMA,C2f_EMA1,C2f_EMA2,C2f_ODConv,SPPF_LSKA,
            VoVGSCSP, VoVGSCSPC, GSConv,
            BiFormerBlock,
            #C2f_iAFF
        }
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        #  ---------------end----------------------
        # ----------------LSKA、CA---------------------
        elif m in {LSKA,CoordAtt,GAM,DAttentionBaseline}:
            args = [ch[f], *args]
        # ----------------end---------------------
        # ----------------FocalModulation----------
        ...
        # ----------------end---------------------
            ...
            c2 = sum([ch[x] for x in f])
        elif m in frozenset({Detect,Detect_DyHead, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect,Detect_DyHead, Segment, Pose, OBB}:
                m.legacy = legacy
